...
ImageManager::ImageManager() {
    ...
    dungeonPixelMap1 = ofImage("images/Maps/dungeonpixelmap1.png");
...
shared_ptr<DynamicEntityAnimations> ImageManager::getRedDarkNutAnimations(){
    DirectionalAnimations movement;
    
    // Tektike's movement animation is the same for all directions
    movement.down = getAnimationFromIndices(50, 51);
    movement.right = getAnimationFromIndices(48, 49);
    movement.left = getAnimationFromIndices(53, 52);
    movement.up = getAnimationFromIndices(16, 34);

    BlueDarkNutAnimations = make_shared<DynamicEntityAnimations>();

    BlueDarkNutAnimations->movement = movement;

    // No attacking animation for Tektike
    BlueDarkNutAnimations->attacking = movement;

    BlueDarkNutAnimations->damage = getAnimationFromIndices(68, 69);

    return BlueDarkNutAnimations;

}
shared_ptr<DynamicEntityAnimations> ImageManager::getMagicTektikeAnimations(){
     DirectionalAnimations movement;
    
    // Tektike's movement animation is the same for all directions
    movement.down = getAnimationFromIndices(89,107);
    movement.right = movement.down;
    movement.left = movement.down;
    movement.up = movement.down;

    magictektikeAnimations = make_shared<DynamicEntityAnimations>();

    magictektikeAnimations->movement = movement;

    // No attacking animation for Tektike
    magictektikeAnimations->attacking = movement;

    magictektikeAnimations->damage = getAnimationFromIndices(89, 107);

    return magictektikeAnimations;
}

...
shared_ptr<Section> MapBuilder::createSection(int startingPixelMapX, int startingPixelMapY, shared_ptr<Link> &linkPtr, ofImage pixelMap) {
    ...
    for (int y = startingPixelMapY; y < startingPixelMapY + VERTICAL_COUNT; y++) {
        ...
        for (int x = startingPixelMapX; x < startingPixelMapX + HORIZONTAL_COUNT; x++) {
            ...
            else if(currentPixel == entityPixelColors.SWORD){
                column = 2;
                row = 4;
                useEntitySheet = true;
                collidable = true;
            ...
            else if (currentPixel == entityPixelColors.DUNGEON1_ENTRANCE) {
                column = 4;
                row = 1;
                isADoor = true;
                whereTo = MapName::DUNGEON1;
            }
            ...
            if (collidable) // We found a block that is collidable; add it to the section
            {
                ...
                else if(currentPixel == entityPixelColors.SWORD)//sword
                {
                       std::cout << "Sword pixel detected at (" << xOffset << ", " << yOffset << ")\n";//debug stuff
                    auto sword = make_shared<Sword>(windowXPos, windowYPos, handler);
                    section->addItemToTile(xOffset, yOffset, sword);
                    //section->addItemToPosition(xOffset, yOffset, sword);
                }
                //Recursive
                else if (currentPixel == tilePixelColors.DUNGEON_UPMOVE_TILE) {
                    block = make_shared<RecursiveMover>(windowXPos, windowYPos, handler, MoveDirection::UP);
                }
                else if (currentPixel == tilePixelColors.DUNGEON_DOWNMOVE_TILE) {
                    block = make_shared<RecursiveMover>(windowXPos, windowYPos, handler, MoveDirection::DOWN);
                }
                else if (currentPixel == tilePixelColors.DUNGEON_LEFTMOVE_TILE) { 
                    block = make_shared<RecursiveMover>(windowXPos, windowYPos, handler, MoveDirection::LEFT);
                }
                else if (currentPixel == tilePixelColors.DUNGEON_RIGHTMOVE_TILE) {
                    block = make_shared<RecursiveMover>(windowXPos, windowYPos, handler, MoveDirection::RIGHT);
                }
                else // nvm it's just a regular block
                {
                    // Handle other blocks (like a regular BoundBlock)
